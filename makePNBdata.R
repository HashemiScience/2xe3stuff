makePNBdata <- function(studentNumber){
  ...
  participant = factor(sample(1:dat_n,dat_n,replace=FALSE), label="person")
  ...
  tmp <- data.frame(participant,
                        quiz1=test1tmp,
                        quiz2=test2tmp,
                        quiz2duration=round(abs(jitter(test2tmp/100*50, dat2_3_r) + rnorm(dat_n,0,4)), 0),
                        EQ=round(rnorm(dat_n, dat4_mean, dat4_sd),0))
  ...
  print(paste0("pnbdata <- makePNBdata(",studentNumber,")"))
